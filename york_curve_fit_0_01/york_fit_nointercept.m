function [b, sigma_b, b_save, mse] = york_fit_nointercept(X, Y, sigma_X, sigma_Y, r)
%[b, sigma_b, b_save] = york_fit_nointercept(X,Y,sigma_X,sigma_Y, r)
...
%Modified by Richard Lange 2021 to remove intercept.
...
[x_hat, y_hat] = york_map_xy(X, Y, sigma_X, sigma_Y, a, b);
mse = mean((X - x_hat).^2 * omega_X + (Y - y_hat).^2 * omega_Y);
