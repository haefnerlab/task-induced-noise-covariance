function [a, b, sigma_a, sigma_b, b_save, mse] = york_fit(X,Y,sigma_X,sigma_Y, r)
...
if nargout >= 6
    assert(all(r == 0), 'mse with correlated errors not implemented');
    mse = mean((X - x).^2 ./ sigma_X.^2 + (Y - y).^2 ./ sigma_Y.^2);
end